  ...
  return (
    <div className={`bg-white rounded-2xl shadow p-4 flex flex-col justify-between relative w-full min-h-[460px] ${className}`}>
      ...
      <div className="absolute top-3 left-3 z-10">
        <span className="text-xs font-bold bg-blue-500 text-white px-2 py-1 rounded-full truncate max-w-[120px]">
      ...
      {product.label && (
        <div className="absolute top-3 right-3 z-10">
          <span className="text-xs font-bold bg-yellow-400 text-white px-2 py-1 rounded-full truncate max-w-[120px]">
            {product.label}
          </span>
        </div>
      )}
      ...
      <div className="flex flex-col gap-2 mb-3">
        ...
        {product.description && (
          <p className="text-sm text-gray-700 line-clamp-1 text-center">
            {product.description}
          </p>
        )}
        ...
        {product.quality && (
          <div className="text-center">
            <span className="inline-block text-xs font-medium bg-green-100 text-green-700 px-3 py-1 rounded-full line-clamp-1">
      ...
      <div className="flex flex-col gap-2 border-t pt-3 text-gray-500 text-sm mt-auto">
        {product.delivery_method && (
          <div className="flex items-center gap-1 line-clamp-1">
            ...
            <span className="truncate">{getDelivery(product.delivery_method)}</span>
          </div>
        )}
        {product.user_id && (
          <div className="flex items-center gap-1 line-clamp-1">
      ...
      <div className="flex items-center justify-between mt-4 gap-2">
        <button
          onClick={handleClick}
          className="flex-1 flex items-center justify-center gap-2 bg-[#18A661] hover:bg-[#149456] text-white text-sm font-semibold px-3 py-2 rounded-full transition truncate"
        >
